...
class Client
{
    ...
    public function getNodeByInnerId($innerId)
    {
        $cached = $this->getEntityCache()->getCachedEntity($innerId, 'node');
        if ($cached) {
            return $cached;
        }else
        {
            $neoQuery = new Cypher\Query($this, "MATCH (product:Product{ProductInnerId:'" . $innerId . "'}) RETURN product");
            $productNode = $this->executeCypherQuery($neoQuery);
            if($productNode->count()>0)
                return $productNode[0];
            else
                return false;
        }
